...
ip = "10.127.3.110"
#ip = "10.127.2.147"
prj_id = "10c87a41313b4b01bd233b02ad25e2f9"
token_id = "gAAAAABgUEtetUXQ7Ym6G3XsfM14BH0aQ7xvmI0UFqBsZbbEEIkJOblzWA73bWTPenUKQgV39e4Npm-SxJ37oSww5wpISbX1YZeIb5bt6ABXAi7vmd8PTIvBo6ulkHIqHD_DkicdEswBZUuyhlVy8ezhXlOngUVrYb-B5mju3H6bBSInR5cMKGM"
...
def curl_keystone():
    url = "https://%s:35357/v3/auth/tokens" % (ip)
    payload='{"auth": {"identity": {"methods": ["password"],"password": {"user": {"name": "admin","domain": {"name": "Default"},"password": "Abc12345"}}}}}'
...
def test_curl_keystone():
    token=curl_keystone()
    print("token:", token)



def list_vpn_endpoint_groups():
    url = "https://%s:9696/v2.0/vpn/endpoint-groups" % (ip)
    requests.packages.urllib3.disable_warnings()
    http = urllib3.PoolManager()
    token = curl_keystone()
    headers = {'Content-Type': 'application/json', 'X-Auth-Token': token }
    try:
        response = http.request('GET',url, body=None, headers=headers)
	print response.data
        print response.headers
        token = ""
...
@mytimeit
def list_cinder_volume():
    url = "https://%s:8776/v3/%s/volumes" % (ip, prj_id)
    requests.packages.urllib3.disable_warnings()
    http = urllib3.PoolManager()
    #token = curl_keystone()
...
@mytimeit
def create_cinder_volume(imagename, imgsize):
    url = "https://%s:8776/v3/%s/volumes" % (ip, prj_id)
    payload = '{"volume": {"status": "creating", "user_id": null, "name": "%s", "imageRef": null, "availability_zone": null, "description": null, "multiattach": false, "attach_status": "detached", "volume_type": null, "metadata": {}, "consistencygroup_id": null, "source_volid": null, "snapshot_id": null, "project_id": null, "source_replica": null, "size": %s}}' % (imagename, imgsize)
    param= json.loads(payload)
    requests.packages.urllib3.disable_warnings()
    http = urllib3.PoolManager()
    #token = curl_keystone()
    ...
    try:
        ...
        if response.status == 202:
            ...
            volume_id = response_body['volume']['id'].encode("utf-8")
            #global volume_id_list
            global volume_id_list
            volume_id_list.append(volume_id)
            print("volume_id_list:%s", volume_id_list)
            return volume_id
    except urllib3.exceptions.MaxRetryError as e:
        print repr(e)

def delete_cinder_volume(volume_id):
    print("volume_id:%s", volume_id)
    url = "https://%s:8776/v3/%s/volumes/%s" % (ip, prj_id, volume_id)
    print("url:%s", url)
    requests.packages.urllib3.disable_warnings()
    http = urllib3.PoolManager()
    #token = curl_keystone()
    headers = {'Content-Type': 'application/json', 'X-Auth-Token': token_id }
    try:
        response = http.request('DELETE',url, body=None, headers=headers)
        print response.data
        print response.headers
        if response.status == 202:
	        print response.data
...
def main():
    #test_curl_keystone()
    #list_vpn_endpoint_groups()
    volume_id = create_cinder_volume('test-cf-volume-4', 1)
    #list_cinder_volume()
    delete_cinder_volume(volume_id)


@mytimeit
def test_create_images_by_api(thread_num, imgsize):
    print("begin test_create_images_by_api===")
    start_time = time.time()
    n = 0
    thread_list = []
    while n < thread_num:
        tmp_imagename = get_temp_image_name()
        t = threading.Thread(target=create_cinder_volume, args=(tmp_imagename, imgsize))
        t.start()
        print("tmp_imagename:%s thread_name:%s started", tmp_imagename, t.getName())
        thread_list.append(t)
        n = n + 1
    ...
    print(">>>>>thread_list all joined:%s", thread_list)
    end_time = time.time()
    print("%s函数平均运行时间为：%.2f" %('test_create_images_by_api', (end_time - start_time)/(thread_num)))
    print("end test_create_images_by_api------------")

@mytimeit
def test_remove_images_by_api(thread_num):
    start_time = time.time()
    n = 0
    thread_list = []
    while n < thread_num:
        global volume_id_list
        print("test_remove_images_by_api volume_id_list:%s", volume_id_list)
        volume_id = volume_id_list.pop()
        print("test_remove_images_by_api after pop volume_id_list:%s volume_id:%s", volume_id_list, volume_id)
        t = threading.Thread(target=delete_cinder_volume, args=(volume_id,))
        t.start()
        thread_list.append(t)
        n = n + 1
    for t in thread_list:
        t.join()
    end_time = time.time()
    print("%s函数平均运行时间为：%.2f" %('test_remove_images_by_api', (end_time - start_time)/(thread_num)))
...
def main_test_2():
    ...
    try:
        times_res = timeit.timeit(
            stmt=partial(test_create_images_by_api, thread_num, 10),
            number=times)
        print("test_create_images:%s", times_res)
    finally:
        times_res = timeit.timeit(
            stmt=partial(test_remove_images_by_api, thread_num),
            number=times)
        print("test_remove_images:%s", times_res)
        #delete_resource()
    # test_create_images(thread_num, ioctx, features)
